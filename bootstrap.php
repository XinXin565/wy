...
$scriptCols=$db->query('PRAGMA table_info(product_scripts)')->fetchAll(PDO::FETCH_COLUMN,1); foreach(['script_hash'=>"TEXT NOT NULL DEFAULT ''",'script_signature'=>"TEXT NOT NULL DEFAULT ''",'published_at'=>"TEXT NOT NULL DEFAULT ''",'previous_source'=>"TEXT NOT NULL DEFAULT ''",'previous_version'=>"TEXT NOT NULL DEFAULT ''"] as $cn=>$ct) if(!in_array($cn,$scriptCols,true)) $db->exec("ALTER TABLE product_scripts ADD COLUMN $cn $ct");

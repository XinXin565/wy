...
$scriptCols=$db->query('PRAGMA table_info(product_scripts)')->fetchAll(PDO::FETCH_COLUMN,1);
foreach(['script_hash'=>"TEXT NOT NULL DEFAULT ''",'script_signature'=>"TEXT NOT NULL DEFAULT ''",'published_at'=>"TEXT NOT NULL DEFAULT ''",'previous_source'=>"TEXT NOT NULL DEFAULT ''",'previous_version'=>"TEXT NOT NULL DEFAULT ''"] as $cn=>$ct) if(!in_array($cn,$scriptCols,true)) $db->exec("ALTER TABLE product_scripts ADD COLUMN $cn $ct");
$db->exec("CREATE TABLE IF NOT EXISTS product_script_versions (id TEXT PRIMARY KEY, product_id TEXT NOT NULL, script_source TEXT NOT NULL, script_version TEXT NOT NULL, script_hash TEXT NOT NULL, script_signature TEXT NOT NULL, created_at TEXT NOT NULL, created_by TEXT NOT NULL DEFAULT '')");
$db->exec("CREATE TABLE IF NOT EXISTS script_execution_audit (id INTEGER PRIMARY KEY AUTOINCREMENT, product_id TEXT, product_code TEXT, function_name TEXT, license_id TEXT, device_id TEXT, script_version TEXT, result TEXT NOT NULL, reason TEXT NOT NULL DEFAULT '', duration_ms INTEGER NOT NULL DEFAULT 0, ip_address TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL)");
...
$cols=$db->query('PRAGMA table_info(licenses)')->fetchAll(PDO::FETCH_COLUMN,1); if(!in_array('key_cipher',$cols,true)) $db->exec('ALTER TABLE licenses ADD COLUMN key_cipher TEXT'); if(!in_array('duration_label',$cols,true)) $db->exec("ALTER TABLE licenses ADD COLUMN duration_label TEXT NOT NULL DEFAULT '永久'"); $deviceCols=$db->query('PRAGMA table_info(devices)')->fetchAll(PDO::FETCH_COLUMN,1); if(!in_array('online_status',$deviceCols,true)) $db->exec("ALTER TABLE devices ADD COLUMN online_status TEXT NOT NULL DEFAULT 'offline'"); if(!in_array('last_run_version',$deviceCols,true)) $db->exec("ALTER TABLE devices ADD COLUMN last_run_version TEXT NOT NULL DEFAULT ''"); $auditCols=$db->query('PRAGMA table_info(audit_logs)')->fetchAll(PDO::FETCH_COLUMN,1); if(!in_array('ip_address',$auditCols,true)) $db->exec("ALTER TABLE audit_logs ADD COLUMN ip_address TEXT NOT NULL DEFAULT ''");
$db->exec("CREATE TABLE IF NOT EXISTS licenses (id TEXT PRIMARY KEY, key_prefix TEXT, key_hash TEXT UNIQUE, key_verify_hash TEXT, product_code TEXT, status TEXT, max_devices INTEGER, expires_at TEXT, created_at TEXT);
CREATE TABLE IF NOT EXISTS devices (id TEXT PRIMARY KEY, license_id TEXT, device_hash TEXT, device_name TEXT, status TEXT, first_seen_at TEXT, last_seen_at TEXT, UNIQUE(license_id, device_hash));
...
function client_ip(): string { $remote=trim((string)($_SERVER['REMOTE_ADDR']??'')); return filter_var($remote,FILTER_VALIDATE_IP)?$remote:''; }

...
std::wstring CloudErrorText(const std::string& body) {
    if (body.find("license_suspended") != std::string::npos) return L"读取失败：卡密已被封禁";
    if (body.find("license_expired") != std::string::npos) return L"读取失败：卡密已到期";
    if (body.find("license_revoked") != std::string::npos) return L"读取失败：卡密已被撤销";
...
void HandleCloud(const std::wstring& message) {
    if (g_license.empty()) {
        SendJson(L"{\"type\":\"cloud\",\"ok\":false,\"message\":\"请先登录授权\"}");
        return;
    }
    try {
        Response cloud = SecureExecute(g_license, wide_to_utf8(message.substr(6)));
        if (cloud.status == 200) {
...
LRESULT CALLBACK AppWndProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    if (message == WM_HEARTBEAT_RESULT) {
        ...
        // 保留卡密上下文，便于授权失效后再次请求云函数并验证服务端拒绝逻辑。
        delete result;
        return 0;
    }

...
(() => {
  ...
  style.textContent = `
    ...
    #licenses>.layui-card,#devices>.layui-card,#logs>.layui-card,#products>.layui-card{background:rgba(255,255,255,.56);border-color:rgba(255,255,255,.78)}
    #licenses>.layui-card>.layui-card-header,#devices>.layui-card>.layui-card-header,#logs>.layui-card>.layui-card-header{display:none}
    #licenses>.layui-card>.layui-card-body,#devices>.layui-card>.layui-card-body,#logs>.layui-card>.layui-card-body{padding:18px}
    #productScriptPanel{margin-top:18px;padding:18px;border:1px solid rgba(255,255,255,.76);border-radius:14px;background:rgba(226,241,244,.46)}
    #productScriptPanel .layui-form-label{color:#395a68;font-weight:650}
    #scriptSource{min-height:240px!important;background:rgba(18,36,46,.92)!important;color:#d9f4ef!important;border-color:rgba(255,255,255,.14)!important;font:13px/1.65 Consolas,"Cascadia Mono",monospace!important;tab-size:2}
    #scriptAuditPanel{padding-top:8px}#scriptAuditPanel h3{margin:8px 0 14px;color:#244553;font-size:16px}#scriptAuditPanel hr{border:0;border-top:1px solid rgba(53,99,113,.1)}
    .p2-empty{padding:42px 20px;text-align:center;color:#82949b}.p2-empty strong{display:block;margin-bottom:6px;color:#49646f;font-size:15px}.p2-loading{position:relative;overflow:hidden;background:rgba(235,243,246,.7)!important;color:transparent!important}.p2-loading:after{content:'';position:absolute;inset:0;background:linear-gradient(100deg,transparent,rgba(255,255,255,.7),transparent);animation:p2Shimmer 1.2s infinite}
    .layui-layer-title{height:52px;line-height:52px;border-bottom-color:rgba(44,88,103,.1);background:rgba(239,248,249,.72);color:#244553;font-weight:650}.layui-layer-content{color:#284550}.layui-layer-btn .layui-layer-btn0{border-color:#1596b5;background:#1596b5}
    .layui-layer.layui-layer-hui{background:rgba(25,48,58,.92)!important;border:1px solid rgba(255,255,255,.2)!important;box-shadow:0 14px 36px rgba(17,42,52,.25)!important;color:#fff!important;backdrop-filter:blur(18px) saturate(130%);-webkit-backdrop-filter:blur(18px) saturate(130%)}
    .layui-layer.layui-layer-hui .layui-layer-content{color:#fff!important;font-weight:600;text-shadow:0 1px 1px rgba(0,0,0,.12)}
    @keyframes p2Shimmer{from{transform:translateX(-100%)}to{transform:translateX(100%)}}
...
(() => {
  const mount = () => {
    const root=document.querySelector('#p2Settings');
    if(!root || document.querySelector('#adminSecurityPanel')) return;
    root.insertAdjacentHTML('beforeend','<div class="layui-col-md12" id="adminSecurityPanel"><div class="layui-card"><div class="layui-card-header">管理员账号与安全入口</div><div class="layui-card-body"><p class="muted">进入后台必须先输入安全入口口令，再验证管理员账号。</p><div style="display:flex;gap:8px;flex-wrap:wrap"><input id="adminCurrentPassword" type="password" class="layui-input" style="width:210px" placeholder="当前管理员密码"><input id="adminNewUsername" class="layui-input" style="width:180px" placeholder="新用户名"><input id="adminNewPassword" type="password" class="layui-input" style="width:210px" placeholder="新密码（留空不修改）"><button id="saveAdminProfile" class="layui-btn layui-btn-sm">保存账号设置</button></div><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:14px"><input id="entryCurrentPassword" type="password" class="layui-input" style="width:210px" placeholder="当前管理员密码"><input id="newEntryPassword" type="password" class="layui-input" style="width:210px" placeholder="新安全入口口令"><button id="saveEntryPassword" class="layui-btn layui-btn-sm layui-btn-normal">保存入口口令</button></div></div></div></div>');
    document.querySelector('#saveAdminProfile').onclick=async()=>{try{const username=document.querySelector('#adminNewUsername').value.trim();const current=document.querySelector('#adminCurrentPassword').value;const next=document.querySelector('#adminNewPassword').value;await adminApiRequest('/admin/profile',{method:'POST',body:JSON.stringify({username,current_password:current,new_password:next})});layui.layer.msg('管理员账号已更新',{icon:1});}catch(e){layui.layer.msg(e.payload?.error==='username_exists'?'用户名已存在':e.payload?.error==='password_too_short'?'新密码至少10位':'账号设置保存失败',{icon:2});}};
    document.querySelector('#saveEntryPassword').onclick=async()=>{try{const current=document.querySelector('#entryCurrentPassword').value;const next=document.querySelector('#newEntryPassword').value;await adminApiRequest('/admin/security-entry',{method:'POST',body:JSON.stringify({current_password:current,new_entry_password:next})});layui.layer.msg('安全入口口令已更新',{icon:1});document.querySelector('#entryCurrentPassword').value='';document.querySelector('#newEntryPassword').value='';}catch(e){layui.layer.msg(e.payload?.error==='entry_password_too_short'?'安全入口口令至少6位':'安全入口口令保存失败',{icon:2});}};
  };
  setInterval(mount,500);setTimeout(mount,250);
})();
(() => { let productId=''; document.addEventListener('click',(e)=>{const b=e.target.closest?.('[data-product-config]');if(b)productId=b.dataset.productConfig||'';},true); setInterval(async()=>{const anchor=document.getElementById('cfgCrypto');if(!anchor||!productId||document.getElementById('productScriptPanel'))return;const panel=document.createElement('div');panel.id='productScriptPanel';panel.className='layui-form-item';panel.innerHTML='<label class="layui-form-label">云函数脚本</label><div class="layui-input-block"><input id="scriptVersion" class="layui-input" value="1" placeholder="脚本版本"><textarea id="scriptSource" class="layui-textarea" style="margin-top:8px;height:180px;font-family:Consolas" placeholder="function get_config(args) { return {}; }"></textarea><label style="display:block;margin-top:8px"><input id="scriptEnabled" type="checkbox"> 启用脚本</label><button type="button" id="saveScript" class="layui-btn layui-btn-sm" style="margin-top:10px">保存脚本</button> <button type="button" id="publishScript" class="layui-btn layui-btn-sm layui-btn-normal" style="margin-top:10px">发布脚本</button></div>';anchor.closest('.layui-form-item')?.after(panel);try{const j=await adminApiRequest('/admin/products/'+productId+'/script',{cache:'no-store'});document.getElementById('scriptSource').value=j.script?.source||'';document.getElementById('scriptVersion').value=j.script?.version||'1';document.getElementById('scriptEnabled').checked=!!j.script?.enabled;}catch(_){}document.getElementById('saveScript').onclick=async()=>{try{await adminApiRequest('/admin/products/'+productId+'/script',{method:'PATCH',body:JSON.stringify({source:document.getElementById('scriptSource').value,version:document.getElementById('scriptVersion').value,enabled:document.getElementById('scriptEnabled').checked})});layui.layer.msg('脚本已保存');}catch(_){layui.layer.msg('脚本保存失败');}};document.getElementById('publishScript').onclick=async()=>{try{await adminApiRequest('/admin/products/'+productId+'/script/publish',{method:'POST',body:'{}'});layui.layer.msg('脚本已发布');}catch(_){layui.layer.msg('脚本发布失败');}};},300); })();
...
(function(){
  ...
  document.addEventListener('click',e=>{const b=e.target.closest?.('[data-product-config]');if(b)currentProductId=b.dataset.productConfig||'';},true);
  const boot=()=>{
    const panel=document.querySelector('#productScriptPanel');
    if(panel&&!panel.querySelector('#testScript')){
      panel.querySelector('#publishScript')?.insertAdjacentHTML('afterend',' <button type="button" id="testScript" class="layui-btn layui-btn-sm layui-btn-warm">测试函数</button> <button type="button" id="rollbackScript" class="layui-btn layui-btn-sm layui-btn-danger">回滚脚本</button> <button type="button" id="historyScript" class="layui-btn layui-btn-sm layui-btn-primary">历史版本</button>');
      document.querySelector('#testScript').onclick=async()=>{if(!currentProductId)return;const html='<div class="layui-form" style="padding:20px"><div class="layui-form-item"><label class="layui-form-label">函数名</label><div class="layui-input-block"><input id="cloudFn" class="layui-input" value="get_security_parameters"></div></div><div class="layui-form-item"><label class="layui-form-label">JSON 参数</label><div class="layui-input-block"><textarea id="cloudArgs" class="layui-textarea">{}</textarea></div></div><button id="runCloudFn" type="button" class="layui-btn layui-btn-fluid">执行测试</button></div>';layui.layer.open({type:1,title:'测试云函数',area:['560px','360px'],content:html,success:(node,index)=>{node.find('#runCloudFn').on('click',async()=>{let args={};try{args=JSON.parse(node.find('#cloudArgs').val()||'{}');}catch(_){return layui.layer.msg('JSON 参数格式错误',{icon:2});}try{const j=await adminApiRequest('/admin/products/'+currentProductId+'/script/test',{method:'POST',body:JSON.stringify({function:node.find('#cloudFn').val().trim(),args})});layui.layer.close(index);layui.layer.alert('<pre>'+esc(JSON.stringify(j.result||j,null,2))+'</pre>',{title:'脚本执行结果'});}catch(e){layui.layer.msg('脚本执行失败',{icon:2});}});}});};
      document.querySelector('#rollbackScript').onclick=async()=>{if(!currentProductId)return;layui.layer.confirm('确认回滚到上一版本？',{title:'回滚脚本',icon:3},async index=>{layui.layer.close(index);try{await adminApiRequest('/admin/products/'+currentProductId+'/script/rollback',{method:'POST',body:'{}'});layui.layer.msg('脚本已回滚');}catch(e){layui.layer.msg('没有可回滚版本',{icon:2});}});};
      document.querySelector('#historyScript').onclick=async()=>{try{const j=await adminApiRequest('/admin/products/'+currentProductId+'/script/versions',{cache:'no-store'});const rows=(j.versions||[]).map(x=>'<tr><td>'+esc(x.script_version)+'</td><td><code>'+esc(x.script_hash)+'</code></td><td>'+esc(fmt(x.created_at))+'</td><td><button class="layui-btn layui-btn-xs versionRollback" data-id="'+esc(x.id)+'">回滚</button></td></tr>').join('')||'<tr><td colspan="4">暂无历史版本</td></tr>';layui.layer.open({type:1,title:'脚本历史版本',area:['720px','460px'],content:'<table class="layui-table"><thead><tr><th>版本</th><th>SHA-256</th><th>创建时间</th><th>操作</th></tr></thead><tbody>'+rows+'</tbody></table>',success:(node,index)=>node.find('.versionRollback').on('click',async function(){const versionId=this.dataset.id;layui.layer.confirm('确认回滚到此版本？',{title:'指定版本回滚',icon:3},async confirmIndex=>{layui.layer.close(confirmIndex);await adminApiRequest('/admin/products/'+currentProductId+'/script/rollback/'+versionId,{method:'POST',body:'{}'});layui.layer.close(index);layui.layer.msg('已回滚指定版本');});})});}catch(e){layui.layer.msg('历史版本读取失败',{icon:2});}};
    }
    const logs=document.querySelector('#logs .layui-card-body');
    if(logs&&!document.querySelector('#scriptAuditPanel')){logs.insertAdjacentHTML('beforeend','<div id="scriptAuditPanel" style="margin-top:24px"><hr><h3>云函数执行审计</h3><div class="toolbar"><input id="scriptAuditProduct" class="layui-input" style="width:180px" placeholder="产品编号"><input id="scriptAuditFn" class="layui-input" style="width:180px" placeholder="函数名"><select id="scriptAuditResult" class="layui-input" style="width:130px"><option value="">全部结果</option><option value="ok">成功</option><option value="fail">失败</option></select><button id="loadScriptAudit" class="layui-btn layui-btn-sm">查询</button></div><table class="layui-table"><thead><tr><th>产品</th><th>函数</th><th>脚本版本</th><th>结果</th><th>原因</th><th>耗时</th><th>IP</th><th>时间</th></tr></thead><tbody id="scriptAuditRows"></tbody></table></div>');
...
// P2 交互收口：统一 Layui 确认、空状态和页面操作，不使用浏览器原生确认框。
(() => {
  const ask=(title,content,onConfirm)=>layui.layer.confirm(content,{title,icon:3,btn:['确认','取消']},async index=>{layui.layer.close(index);await onConfirm();});
  window.removeKey=id=>ask('删除卡密','删除后不可恢复，确认继续？',async()=>{try{await adminApiRequest('/admin/licenses/'+id,{method:'DELETE'});layui.layer.msg('卡密已删除',{icon:1});window.refreshAdminData?.();}catch(_){layui.layer.msg('删除失败',{icon:2});}});
  window.bulkDelete=()=>{const ids=[...document.querySelectorAll('.keyCheck:checked')].map(x=>x.value);if(!ids.length)return layui.layer.msg('请先勾选卡密');ask('批量删除','确认删除选中的 '+ids.length+' 张卡密？',async()=>{try{await adminApiRequest('/admin/licenses/bulk-delete',{method:'POST',body:JSON.stringify({ids})});layui.layer.msg('已删除 '+ids.length+' 张卡密',{icon:1});window.refreshAdminData?.();}catch(_){layui.layer.msg('批量删除失败',{icon:2});}});};
  const emptyStates=()=>{[['licenseRows',9,'当前筛选条件下没有卡密'],['deviceRows',8,'当前没有设备记录'],['logRows',8,'当前没有审计记录'],['scriptAuditRows',8,'当前没有云函数执行记录']].forEach(([id,cols,text])=>{const body=document.getElementById(id);if(!body||body.children.length)return;body.innerHTML='<tr><td colspan="'+cols+'"><div class="p2-empty"><strong>暂无数据</strong>'+text+'</div></td></tr>';});};
  const improveActions=()=>{const logActions=document.querySelector('#logsPageActions');if(logActions&&!document.querySelector('#p2RefreshLogs')){logActions.innerHTML='<button type="button" class="layui-btn layui-btn-sm layui-btn-primary" id="p2RefreshLogs">刷新日志</button>';document.querySelector('#p2RefreshLogs').onclick=()=>window.renderAudit?.();}const deviceActions=document.querySelector('#devicesPageActions');if(deviceActions&&!document.querySelector('#p2RefreshDevices')){deviceActions.innerHTML='<button type="button" class="layui-btn layui-btn-sm layui-btn-primary" id="p2RefreshDevices">刷新设备</button>';document.querySelector('#p2RefreshDevices').onclick=()=>window.refreshAdminData?.();}};
  setInterval(()=>{emptyStates();improveActions();},900);
})();

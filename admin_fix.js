...
(function(){
  ...
  const boot=()=>{
    ...
    if(panel&&!panel.querySelector('#testScript')){
      panel.querySelector('#publishScript')?.insertAdjacentHTML('afterend',' <button type="button" id="testScript" class="layui-btn layui-btn-sm layui-btn-warm">测试函数</button> <button type="button" id="rollbackScript" class="layui-btn layui-btn-sm layui-btn-danger">回滚脚本</button>');
      document.querySelector('#testScript').onclick=async()=>{const fn=prompt('请输入函数名','get_security_parameters');if(!fn||!currentProductId)return;try{const j=await adminApiRequest('/admin/products/'+currentProductId+'/script/test',{method:'POST',body:JSON.stringify({function:fn,args:{}})});layui.layer.alert('<pre>'+esc(JSON.stringify(j.result||j,null,2))+'</pre>',{title:'脚本执行结果'});}catch(e){layui.layer.msg('脚本执行失败',{icon:2});}};
